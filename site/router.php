...
/**
 * Routing class from com_properties
 *
 * @package     Realeza
 * @subpackage  com_properties
 * @since       1.0
...
class PropertiesRouter extends JComponentRouterBase
{
	...
	public function build(&$query)
	{
		...
		if (isset($query['view']))
		{
			...
			if (empty($query['Itemid']) || empty($menuItem) || $menuItem->component != 'com_properties')
			{
				$segments[] = $query['view'];
			}
		...
		if (isset($view) and ($view == 'category' or $view == 'property'))
		{
			if ($mId != (int) $query['id'] || $mView != $view)
			{
				if ($view == 'property' && isset($query['catid']))
				{
					$catid = $query['catid'];
				}
